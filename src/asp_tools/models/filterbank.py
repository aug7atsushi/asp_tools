...
from asp_tools.utils.audio import build_optimal_window, build_window
...
class FourierEncoder(nn.Module):
    def __init__(
        ...
        self.frequency, self.time_seq = nn.Parameter(
            omega, requires_grad=trainable
        ), nn.Parameter(n, requires_grad=False)
    ...
    def forward(self, input):
        ...
        if self.trainable_phase:
            phi = self.phase
            basis_real, basis_imag = torch.cos(
                -(omega_n + phi.unsqueeze(dim=1))
            ), torch.sin(-(omega_n + phi.unsqueeze(dim=1)))
        else:
            basis_real, basis_imag = torch.cos(-omega_n), torch.sin(-omega_n)
        basis_real, basis_imag = basis_real.unsqueeze(dim=1), basis_imag.unsqueeze(
            dim=1
        )

        if not self.onesided:
            _, basis_real_conj, _ = torch.split(
                basis_real, [1, n_basis // 2 - 1, 1], dim=0
            )
            _, basis_imag_conj, _ = torch.split(
                basis_imag, [1, n_basis // 2 - 1, 1], dim=0
            )
            basis_real_conj, basis_imag_conj = torch.flip(
                basis_real_conj, dims=(0,)
            ), torch.flip(basis_imag_conj, dims=(0,))
            basis_real, basis_imag = torch.cat(
                [basis_real, basis_real_conj], dim=0
            ), torch.cat([basis_imag, -basis_imag_conj], dim=0)
        basis_real, basis_imag = window * basis_real, window * basis_imag
        output_real, output_imag = F.conv1d(input, basis_real, stride=stride), F.conv1d(
            input, basis_imag, stride=stride
        )
        output = torch.cat(
            [output_real.unsqueeze(dim=-1), output_imag.unsqueeze(dim=-1)], dim=-1
        )
...
class FourierDecoder(nn.Module):
    ...
    def forward(self, input):
        ...
        if torch.is_complex(input):
            ...
            n_bins = input.size(1)
            input_real, input_imag = torch.split(
                input, [n_bins // 2, n_bins // 2], dim=1
            )
        ...
        basis_real, basis_imag = (
            optimal_window * basis_real,
            optimal_window * basis_imag,
        )
        basis_real, basis_imag = basis_real / n_basis, basis_imag / n_basis

        if self.onesided:
            _, input_real_conj, _ = torch.split(
                input_real, [1, n_basis // 2 - 1, 1], dim=1
            )
            _, input_imag_conj, _ = torch.split(
                input_imag, [1, n_basis // 2 - 1, 1], dim=1
            )
            input_real_conj, input_imag_conj = torch.flip(
                input_real_conj, dims=(1,)
            ), torch.flip(input_imag_conj, dims=(1,))
            input_real, input_imag = torch.cat(
                [input_real, input_real_conj], dim=1
            ), torch.cat([input_imag, -input_imag_conj], dim=1)

        output = F.conv_transpose1d(
            input_real, basis_real, stride=stride
        ) - F.conv_transpose1d(input_imag, basis_imag, stride=stride)
...
class PinvDecoder(nn.Module):
    def __init__(self, encoder: Union[Encoder, FourierEncoder]):
        ...
        if isinstance(encoder, Encoder):
            ...
            if n_rows < n_columns:
                raise ValueError(
                    "Cannot compute the left inverse of encoder's weight. In encoder, `out_channels` must be equal to or greater than `kernel_size`."
                )
        elif isinstance(encoder, FourierEncoder):
            if encoder.onesided or encoder.return_complex:
                raise ValueError(
                    "Both encoder.onesided and encoder.return_complex are expected to be False."
                )
        else:
    ...
    def forward(self, input):
        ...
        if isinstance(encoder, Encoder):
            ...
            weight_pinverse = (
                torch.pinverse(weight).permute(2, 0, 1).contiguous() / duplicate
            )
...
def _test_filterbank():
    ...
    plt.figure()
    plt.pcolormesh(
        enc_basis.squeeze(dim=1).detach().cpu().numpy(),
        cmap="bwr",
    ...
    plt.figure()
    plt.pcolormesh(
        dec_basis.squeeze(dim=1).detach().cpu().numpy(),
        cmap="bwr",
        norm=Normalize(vmin=-1, vmax=1),
    )
    plt.colorbar()
    ...
    encoder = FourierEncoder(
        n_basis, kernel_size, stride=stride, onesided=True, return_complex=True
    )
    ...
    encoder = FourierEncoder(
        n_basis, kernel_size, stride=stride, onesided=False, return_complex=True
    )
    ...
    encoder = FourierEncoder(
        n_basis, kernel_size, stride=stride, onesided=False, return_complex=False
    )
    decoder = FourierDecoder(n_basis, kernel_size, stride=stride, onesided=False)
...
def _test_fourier():
    ...
    spectrogram_stft = torch.stft(
        input.view(batch_size * C, T),
        kernel_size,
        hop_length=stride,
        window=window,
        center=False,
    ...
    decoder = FourierDecoder(
        n_basis, kernel_size, stride=stride, window_fn="hann", onesided=onesided
    )
    spectrogram = encoder(input)
    output = decoder(spectrogram)
    ...
    plt.plot(output[0, 0].detach().numpy())
    plt.savefig(
        "data/filterbank/reconstruction_N{}_K{}.png".format(n_basis, kernel_size),
        bbox_inches="tight",
    )
    plt.close()
