...
from asp_tools.models.tdcn import TimeDilatedConvNet
from asp_tools.utils.filterbank import choose_filterbank
from asp_tools.utils.model import choose_nonlinear
from asp_tools.utils.tasnet import choose_layer_norm
...
class ConvTasNet(nn.Module):
    pretrained_model_ids = {
        "wsj0-mix": {
            8000: {
                ...
                3: {"enc_relu": "1-4Abl7LnEtwqMnAFQOcNLUOaDbgp3NoG"},
            },
            16000: {2: "", 3: ""},  # TODO  # TODO
        },
        "wham/enhance-single": {
        ...
        "librispeech": {
            SAMPLE_RATE_LIBRISPEECH: {2: "1NI6Q_WZHiTKkgkNTEcZE1yHskHgYUHpy"}
        },
    }
    ...
    def __init__(
        ...
        enc_basis: str | None = None,
        dec_basis: str | None = None,
        ...
        **kwargs,
        ...
        if enc_basis in [
            ...
            self.enc_onesided, self.enc_return_complex = (
                kwargs["enc_onesided"],
                kwargs["enc_return_complex"],
            )
        else:
            self.window_fn = None
        ...
        (
            self.sep_hidden_channels,
            self.sep_bottleneck_channels,
            self.sep_skip_channels,
    ...
    def extract_latent(self, input):
        ...
        if n_dims == 3:
            ...
            assert C_in == 1, "input.size() is expected (?, 1, ?), but given {}".format(
                input.size()
            )
        elif n_dims == 4:
    ...
    def build_model(cls, model_path, load_state_dict=False):
        ...
        enc_basis, dec_basis = (
            config.get("enc_bases") or config["enc_basis"],
            config.get("dec_bases") or config["dec_basis"],
        )
        enc_nonlinear = config["enc_nonlinear"]
        enc_onesided, enc_return_complex = (
            config.get("enc_onesided") or None,
            config.get("enc_return_complex") or None,
        )
        window_fn = config["window_fn"]

        sep_hidden_channels, sep_bottleneck_channels, sep_skip_channels = (
            config["sep_hidden_channels"],
            config["sep_bottleneck_channels"],
            config["sep_skip_channels"],
        )
        sep_kernel_size = config["sep_kernel_size"]
        sep_num_blocks, sep_num_layers = (
            config["sep_num_blocks"],
            config["sep_num_layers"],
        )

        dilated, separable, causal = (
            config["dilated"],
            config["separable"],
            config["causal"],
        )
    ...
    def build_from_pretrained(
        cls, root="./pretrained", quiet=False, load_state_dict=True, **kwargs
    ):
        ...
        if task in ["wsj0-mix", "wsj0"]:
            ...
            download_dir = os.path.join(
                ...
                cls.__name__,
                task,
                "sr{}/{}speakers/{}".format(sample_rate, n_sources, config),
            )

            additional_attributes.update({"n_sources": n_sources})
        elif task == "musdb18":
            ...
            download_dir = os.path.join(
                root, cls.__name__, task, "sr{}".format(sample_rate), config
            )
        elif task in [
            "wham/separate-noisy",
            "wham/enhance-single",
            ...
            download_dir = os.path.join(
                root, cls.__name__, task, "sr{}".format(sample_rate)
            )
            ...
            download_dir = os.path.join(
                ...
                cls.__name__,
                task,
                "sr{}/{}speakers".format(sample_rate, n_sources),
            )
        ...
        additional_attributes.update({"sample_rate": sample_rate})

        model_path = os.path.join(download_dir, "model", "{}.pth".format(model_choice))

        if not os.path.exists(model_path):
            download_pretrained_model_from_google_drive(
                model_id, download_dir, quiet=quiet
            )
        ...
        if task == "musdb18":
            additional_attributes.update(
                {"sources": config["sources"], "n_sources": len(config["sources"])}
            )
...
def _test_conv_tasnet():
    ...
    model = ConvTasNet(
        ...
        sep_bottleneck_channels=B,
        sep_skip_channels=Sc,
        sep_kernel_size=P,
        sep_num_blocks=R,
        sep_num_layers=X,
        causal=causal,
    ...
    plt.figure()
    plt.pcolormesh(
        basis.squeeze(dim=1).detach(), cmap="bwr", norm=Normalize(vmin=-1, vmax=1)
    )
    plt.colorbar()
    plt.savefig("outputs/conv-tasnet/basis_enc-Fourier.png", bbox_inches="tight")
    plt.close()
...
def _test_conv_tasnet_paper():
    ...
    plt.colorbar()
    plt.savefig("outputs/conv-tasnet/basis_enc-trainable.png", bbox_inches="tight")
    plt.close()
