from asp_tools.models.filterbank import (
    Decoder,
    Encoder,
    FourierDecoder,
    FourierEncoder,
...
def choose_filterbank(
    ...
    **kwargs
    ...
    if enc_basis == "trainable":
        ...
        trainable_phase = (
            True if enc_basis == "trainableFourierTrainablePhase" else False
        )
        onesided, return_complex = bool(kwargs["enc_onesided"]), bool(
            kwargs["enc_return_complex"]
        )
        window_fn = kwargs["window_fn"]
        n_basis = compute_valid_basis(
            hidden_channels, onesided=onesided, return_complex=return_complex
        )
        encoder = FourierEncoder(
            ...
            window_fn=window_fn,
            trainable=trainable,
            trainable_phase=trainable_phase,
            onesided=onesided,
    ...
    if dec_basis == "trainable":
        ...
        trainable_phase = (
            True if dec_basis == "trainableFourierTrainablePhase" else False
        )
        onesided, return_complex = bool(kwargs["enc_onesided"]), bool(
            kwargs["enc_return_complex"]
        )
...
def assert_monoral(in_channels):
    # TODO: stereo input
    assert in_channels == 1, "`in_channels` is expected 1, but given {}.".format(
        in_channels
    )
